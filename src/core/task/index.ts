		...
		try {
			...
			if (hooksEnabled && shouldRunTaskCancelHook) {
				try {
					...
					// This allows abortTask() to complete so future cancels work
					this.initiateTaskLoop(newUserContent.length > 0 ? newUserContent : [])

					// Don't wait for task to complete - return immediately
					// This clears isAborting flag so user can cancel again
				} catch (error) {
